...
fun calculateInsights(
    ...
    /** Uses groupingBy().eachCount() for O(N) grouping without intermediate map/list allocations. */
    ...
    /** Groups and counts daily completions efficiently via groupingBy().eachCount() */
...
fun calculateCapacitySnapshot(
    ...
    /** distinctBy avoids instantiating grouped lists just to count unique keys, lowering memory overhead. */
