...
package com.tajemniktv.tajos.ui.main.calculators

import com.tajemniktv.tajos.data.*
...
import com.tajemniktv.tajos.ui.main.state.CoreLifeOSShiftItem
import com.tajemniktv.tajos.ui.main.state.CoreLifeOSShiftSnapshot
import com.tajemniktv.tajos.ui.main.state.DirectionCommitmentStatus
import com.tajemniktv.tajos.ui.main.state.DistinctionQuestionState
...
import com.tajemniktv.tajos.ui.main.state.LoadTrendPoint
...
import com.tajemniktv.tajos.ui.main.state.PlaceLogisticsItem
import com.tajemniktv.tajos.ui.main.state.PlaybookSnapshot
import com.tajemniktv.tajos.ui.main.state.RelationshipSnapshot
import com.tajemniktv.tajos.ui.main.state.RelationshipStatusItem
import com.tajemniktv.tajos.ui.main.state.StudentBoardState
import com.tajemniktv.tajos.ui.main.state.StudentCourseSummary
import com.tajemniktv.tajos.ui.main.state.StudentMasteryItem
import com.tajemniktv.tajos.ui.main.state.StudentProgressItem
import com.tajemniktv.tajos.ui.main.state.StudentSemesterSummary
...
fun calculateAreaHealthSnapshot(
...
): com.tajemniktv.tajos.ui.AreaHealthSnapshot {
    if (areas.isEmpty()) {
        return com.tajemniktv.tajos.ui
            .AreaHealthSnapshot()
    }
    ...
    val computed =
        areas
            .map { area ->
                ...
                com.tajemniktv.tajos.ui.AreaHealthMetrics(
                    areaId = area.id,
    ...
    return com.tajemniktv.tajos.ui.AreaHealthSnapshot(
        areas = computed,
...
fun calculateOpenLoopsSnapshot(
...
): com.tajemniktv.tajos.ui.OpenLoopsSnapshot {
    val now = Clock.System.now().toEpochMilliseconds()
    ...
    val openLoopItems =
        nodes
            ...
            .map { openLoop ->
                ...
                com.tajemniktv.tajos.ui.OpenLoopStatusItem(
                    node = openLoop,
                    urgency = urgency,
                    ageDays = ageDays,
    ...
    return com.tajemniktv.tajos.ui.OpenLoopsSnapshot(
        active = active,
        inbox = inbox,
        review = review,
        resolved = resolved,
 ...
 */
fun calculateMaintenanceSnapshot(nodes: List<NodeWithPin>): com.tajemniktv.tajos.ui.MaintenanceSnapshot {
    val now = Clock.System.now().toEpochMilliseconds()
    ...
    val activeItems =
        nodes
            ...
            .map { item ->
                ...
                com.tajemniktv.tajos.ui.MaintenanceStatusItem(
                    node = item,
                    urgency = urgency,
    ...
    return com.tajemniktv.tajos.ui.MaintenanceSnapshot(
        active = activeItems,
        recurring = recurring,
        overdue = overdue,
        byType = byType,
...
fun calculateTimeArchitectureSnapshot(
    ...
    projects: List<NodeEntity>,
): com.tajemniktv.tajos.ui.TimeArchitectureSnapshot {
    val now = Clock.System.now().toEpochMilliseconds()
    ...
    val countdowns =
        dueNodes
            .map { item ->
                ...
                com.tajemniktv.tajos.ui.TimeCountdownItem(
                    node = item,
                    daysLeft = daysLeft,
                )
            }.take(8)
    ...
    val projectPhases =
        projects
            .map { project ->
                ...
                com.tajemniktv.tajos.ui.ProjectPhaseItem(
                    project = project,
    ...
    return com.tajemniktv.tajos.ui.TimeArchitectureSnapshot(
...
fun calculateCapacitySnapshot(
    ...
    projects: List<NodeEntity>,
    areas: List<NodeEntity>,
    maintenance: com.tajemniktv.tajos.ui.MaintenanceSnapshot,
    openLoops: com.tajemniktv.tajos.ui.OpenLoopsSnapshot,
...
fun calculateLifeOSSignatureSnapshot(
    ...
    areaHealth: com.tajemniktv.tajos.ui.AreaHealthSnapshot,
    openLoops: com.tajemniktv.tajos.ui.OpenLoopsSnapshot,
...
fun calculateLifeOSSecondBrainSnapshot(
    nodes: List<NodeWithPin>,
    relations: List<RelationEntity>,
    dashboard: com.tajemniktv.tajos.ui.DashboardUIState,
    areaHealth: com.tajemniktv.tajos.ui.AreaHealthSnapshot,
    openLoops: com.tajemniktv.tajos.ui.OpenLoopsSnapshot,
    maintenance: com.tajemniktv.tajos.ui.MaintenanceSnapshot,
    capacity: CapacitySnapshot,
...
fun calculateCoreLifeOSShiftSnapshot(
    ...
    dashboard: com.tajemniktv.tajos.ui.DashboardUIState,
    time: com.tajemniktv.tajos.ui.TimeArchitectureSnapshot,
    areaHealth: com.tajemniktv.tajos.ui.AreaHealthSnapshot,
    openLoops: com.tajemniktv.tajos.ui.OpenLoopsSnapshot,
    maintenance: com.tajemniktv.tajos.ui.MaintenanceSnapshot,
    protocols: TransitionProtocolsSnapshot,
