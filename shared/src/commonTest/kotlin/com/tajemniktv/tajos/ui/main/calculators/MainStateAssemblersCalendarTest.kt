...
/** Guards the calendar projection across explicit schedules, legacy dates, and external events. */
class MainStateAssemblersCalendarTest {

    @Test
    fun `explicit schedules suppress legacy duplicates and merge chronologically with external events`() {
        val node = createTestNodeWithPin(defaultNode(1).copy(dueAt = 500L, content = ""))
        val entries = buildCalendarEntries(
            nodes = listOf(node),
            scheduleEntries = listOf(
                ScheduleEntryEntity(id = 1, itemId = 1, scheduledAt = 2000L, kind = "due", note = "Schedule details"),
                ScheduleEntryEntity(id = 2, itemId = 999, scheduledAt = 100L, kind = "due"),
            ),
            externalEvents = listOf(CalendarEventEntity(id = 3, providerId = 1, title = "Meeting", startAt = 1000L, endAt = 1500L)),
        )
        assertEquals(listOf("ext_3", "schedule_1"), entries.map { it.id })
        assertEquals("Schedule details", entries.last().description)
    }
