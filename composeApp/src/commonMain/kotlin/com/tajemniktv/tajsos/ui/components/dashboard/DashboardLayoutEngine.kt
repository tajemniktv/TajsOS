...
private val dashboardBlockSpecs: List<DashboardBlockSpec> =
    listOf(
        ...
        DashboardBlockSpec("today_pulse", aliases = setOf("today_top_3")),
        ...
        DashboardBlockSpec("focus", aliases = setOf("current_task", "timer")),
        DashboardBlockSpec("insights"),
        DashboardBlockSpec("actions"),
        DashboardBlockSpec("suggestions", aliases = setOf("easy_wins")),
        DashboardBlockSpec("knowledge", aliases = setOf("pinned_note")),
        ...
        DashboardBlockSpec("search_capture", supportedSurfaces = setOf(DashboardSurface.DESKTOP)),
        DashboardBlockSpec("insights_summary", supportedSurfaces = setOf(DashboardSurface.DESKTOP)),
        DashboardBlockSpec("system_clock", supportedSurfaces = setOf(DashboardSurface.DESKTOP)),
        DashboardBlockSpec("system_footer"),
        DashboardBlockSpec("command_bar", supportedSurfaces = setOf(DashboardSurface.DESKTOP)),
    )
...
fun dashboardBlockCatalog(): List<DashboardBlockCatalogEntry> =
    dashboardBlockSpecs
        .map { DashboardBlockCatalogEntry(id = it.id, aliases = it.aliases) }
        .sortedBy { it.id }
...
fun buildDashboardLayoutPlan(
    ...
    val explicitLayout = parseVersionedLayout(preferenceJson)
    if (explicitLayout != null) {
        return DashboardLayoutPlan(
            primary = normalizeBlocks(explicitLayout.primary, surface, enabledPacks),
            secondary = normalizeBlocks(explicitLayout.secondary, surface, enabledPacks),
            footer = normalizeBlocks(explicitLayout.footer, surface, enabledPacks),
            bottomBar = normalizeBlocks(explicitLayout.bottomBar, surface, enabledPacks),
        )
    }

    val preferenceBlocks = parseLegacyBlockList(preferenceJson)
    val modeBlocks = fromModeProfile.ifEmpty { preferenceBlocks }
    val dynamicContent = normalizeBlocks(modeBlocks, surface, enabledPacks)
    return when (surface)
    {
        DashboardSurface.MOBILE -> {
            val fallback = normalizeBlocks(defaultMobilePrimaryIds, surface, enabledPacks)
            val ordered =
            ...
            DashboardLayoutPlan(primary = ordered)
        }
        ...
        DashboardSurface.DESKTOP -> {
            val primary =
                if (dynamicContent.isNotEmpty()) {
                    ...
                    normalizeBlocks(defaultDesktopPrimaryIds, surface, enabledPacks)
                }
            DashboardLayoutPlan(
                primary = primary,
                secondary = normalizeBlocks(defaultDesktopSecondaryIds, surface, enabledPacks),
                bottomBar = normalizeBlocks(defaultDesktopBottomBarIds, surface, enabledPacks),
            )
        }
    }
}
...
private fun normalizeBlocks(
    ...
    return ids
        .mapNotNull { raw ->
            ...
            val spec = blockSpecById[normalized] ?: return@mapNotNull null
            if (!spec.supportedSurfaces.contains(surface)) return@mapNotNull null
            if (!spec.isEnabled(enabledPacks)) return@mapNotNull null
            DashboardBlockInstance(spec.id)
        }.distinctBy { it.id }
}

private fun parseLegacyBlockList(raw: String?): List<String> {
    if (raw.isNullOrBlank()) return emptyList()
    return runCatching { dashboardJson.decodeFromString<List<String>>(raw) }.getOrElse { emptyList() }
}

private fun parseVersionedLayout(raw: String?): DashboardLayoutJsonV1? {
    if (raw.isNullOrBlank()) return null
    return runCatching { dashboardJson.decodeFromString<DashboardLayoutJsonV1>(raw) }.getOrNull()
}
