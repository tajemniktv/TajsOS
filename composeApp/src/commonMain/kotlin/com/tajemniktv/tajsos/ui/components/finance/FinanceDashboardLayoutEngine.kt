...
package com.tajemniktv.tajsos.ui.components.finance
...
fun buildFinanceDashboardPlan(
    ...
    if (parsed != null) {
        return FinanceDashboardPlan(
            primary = parsed.primary.distinct().map { FinanceDashboardBlock(it) },
            secondary = parsed.secondary.distinct().map { FinanceDashboardBlock(it) },
        )
    }
    ...
    return when (surface)
    {
        FinanceDashboardSurface.MOBILE -> {
            FinanceDashboardPlan(primary = mobileDefaults.map { FinanceDashboardBlock(it) })
        }

        FinanceDashboardSurface.DESKTOP -> {
            FinanceDashboardPlan(
                primary = desktopPrimaryDefaults.map { FinanceDashboardBlock(it) },
                secondary = desktopSecondaryDefaults.map { FinanceDashboardBlock(it) },
            )
        }
    }
}
...
private fun parseStructured(raw: String?): FinanceLayoutJsonV1? {
    ...
    return runCatching { financeLayoutJson.decodeFromString<FinanceLayoutJsonV1>(raw) }.getOrNull()
}
