...
## TajsOS Integration Note

Use Navigation 3 patterns in a way that preserves TajsOS screen architecture:

- Keep `AppShell` as the owner of persistent chrome and route-host placement.
- Keep `ScreenScaffold`/`SplitScreenScaffold` as the owner of screen-level structure and insets.
- Adapt recipes to TajsOS shell and screen boundaries instead of replacing them.
...
- *[Common UI](references/android/guide/navigation/navigation-3/recipes/common-ui.md)*: Demonstrates
  how to implement a common navigation UI pattern with a bottom navigation bar and multiple back
  stacks, where each tab in the navigation bar has its own navigation history.
...
- *[Dialog](references/android/guide/navigation/navigation-3/recipes/dialog.md)*: Shows how to
  create a Dialog.
...
- *[Animations](references/android/guide/navigation/navigation-3/recipes/animations.md)*: Shows how
  to override the default animations for all destinations and a single destination.
...
level route, and the navigation state persists config changes and process death.
