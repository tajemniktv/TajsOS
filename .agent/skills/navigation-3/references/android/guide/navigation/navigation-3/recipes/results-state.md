...
1. **`ResultStore`** : A `ResultStore` is created and made available to the composables. This store
   holds the results.
2. **Setting the result** : The screen that produces the result calls
   `resultStore.setResult(person)` to save the data in the store.
3. **Observing the result** : The screen that needs the result calls
   `resultStore.getResultState<Person?>()` to get a `State` object representing the result. The UI
   then observes this state and recomposes whenever the result changes.

This approach is suitable when the result should be treated as persistent state that survives
