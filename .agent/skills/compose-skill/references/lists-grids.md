...
Only compose visible items — use for large or dynamic lists. For small fixed lists (<10 items),
prefer `Column`/`Row`.
...
**Rule:** Use domain IDs, not indices. Without stable keys, removing an item corrupts the state of
remaining items.
...
Without `contentType`, all items compete for one reuse pool. With it, items reuse layout state
efficiently within their type.
