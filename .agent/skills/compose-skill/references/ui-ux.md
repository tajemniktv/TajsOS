...
Utility apps are trust products. The UI must feel: stable, immediate, precise, reversible,
non-destructive.
...
- **Spinner**: only for small unknown-layout operations or blocking tasks with no stable placeholder
  shape
...
Bad disabled state: button disabled with no visible reason, form cleared during loading, entire
screen grayed out for a small refresh.
