...
- Generate brand colors via [Material Theme Builder](https://m3.material.io/theme-builder) for
  guaranteed tonal palettes.
...
**Default**: Use `NavigationSuiteScaffold` for apps with 3-5 top-level destinations. It adapts
automatically.
...
**MVI pattern**: ViewModel emits `Effect.ShowSheet`; Route composable calls `sheetState.show()` in
`LaunchedEffect`.
...
**MVI pattern**: Dialog visibility controlled by `state.showDialog: Boolean`. Confirm/dismiss
dispatch events.
...
**Rule**: Compute `WindowSizeClass` once at app/activity level via `currentWindowAdaptiveInfo()`.
Pass derived layout decisions down as state.
...
**Default**: For list-detail apps, use `NavigableListDetailPaneScaffold` which handles pane
visibility and back navigation.

**Adaptive navigation:** read `windowSizeClass` (or related adaptive info) once at the root and pass
derived flags (e.g. whether to show a top app bar) into your main screen composable.
...
**Key change**: M3 `Scaffold` no longer has `drawerState`. Use `ModalNavigationDrawer` wrapping
`Scaffold` instead.
