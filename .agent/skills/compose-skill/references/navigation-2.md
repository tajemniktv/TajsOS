...
NavHost, NavController, and graph DSL for Jetpack Compose navigation. Nav 2 is **not deprecated**
...
How you wire ViewModels and state inside each `composable` block depends on your project's
architecture — see [navigation.md](navigation.md) for the MVI boundary pattern where navigation is
driven by ViewModel effects.

## Type-Safe Routes (2.8+)

From Navigation Compose 2.8+, routes can be `@Serializable` types instead of strings. This is the
recommended approach for new Nav 2 code:
...
Use `NavigationBar` with `currentBackStackEntryAsState()`. Track selection with
`destination.hierarchy` and `hasRoute(route::class)`.
...
Type-safe: use `navigation<Graph>(startDestination = Route)` with `@Serializable` types — same
structure as above.
