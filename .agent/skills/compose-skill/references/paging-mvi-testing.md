...
MVI dual-flow pattern for paging, testing strategies, and anti-patterns. This file builds on the
core Paging setup in [paging.md](paging.md).
...
PagingData must be a **separate Flow** from the MVI ViewModel state. The ViewModel handles
non-paging concerns (filters, selection mode, errors). PagingData flows independently.
...
The route composable collects both the MVI state and PagingData flow, then passes them to the
stateless screen composable. Use a DI-agnostic ViewModel parameter.
...
The screen composable is dumb — it receives `LazyPagingItems` and state as props, emits events as
callbacks.
